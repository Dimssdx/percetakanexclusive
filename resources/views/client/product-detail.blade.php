        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
                ...
                <div class="relative w-full rounded-xl overflow-hidden bg-gray-100 mb-4" style="padding-top: 100%;">
                    <img id="mainImage" src="{{ $gambarUtama ?? $placeholderImg }}"
                    ...
                    @if ($hasMultipleImages)
                        <div
                            class="absolute top-4 right-4 bg-black/60 text-white px-3 py-1 rounded-full text-sm font-semibold">
                ...
                @if ($hasMultipleImages)
                    <div class="grid grid-cols-4 gap-2">
                        @foreach ($gambarUrls as $index => $imageUrl)
                            <div class="thumbnail {{ $index === 0 ? 'active' : '' }} relative rounded-lg overflow-hidden bg-gray-100"
                                style="padding-top: 100%;"
                                onclick="changeMainImage('{{ addslashes($imageUrl) }}', {{ $index }})">
                                <img src="{{ $imageUrl }}" alt="Gambar {{ $index + 1 }}"
