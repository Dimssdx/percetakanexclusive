...
<head>
    ...
    <style>
        ...
        @media (max-width: 1060px) { .custom-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); } }
        @media (max-width: 780px) { .custom-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
        @media (max-width: 480px) { .custom-grid { grid-template-columns: repeat(1, minmax(0, 1fr)); } }
    </style>
</head>
        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div id="product-grid" class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 custom-grid">
                <div id="product-loading" class="col-span-full text-center text-gray-500 py-12">
                    <svg class="animate-spin h-8 w-8 text-pink-600 mx-auto mb-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            <div id="load-more-container" class="flex justify-center mt-12 hidden">
                <button id="load-more-btn" data-next-page="2" class="px-6 py-3 rounded-full bg-gray-900 text-white text-sm font-semibold hover:bg-pink-600 transition disabled:opacity-50 disabled:cursor-not-allowed">
